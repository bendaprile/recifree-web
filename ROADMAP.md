...
- ✅ **Paste Deduplication:** A URL that already maps to a published recipe never produces a second one. `publishRecipe.js` stores `sourceUrlHash` on every recipe it publishes; `functions/publishedLookup.js` matches a paste against it, and `extractRecipe.js` answers `{ duplicate: true, slug }` instead of extracting. `AddRecipe` routes the user to that recipe and adds it to their saved list, and the recipe page says why they landed there.
  ...
  - *The key is the same normalized URL hash the extraction cache uses*, imported rather than reimplemented, so a trailing slash, a `?utm_source=`, and a reordered query string all match. A second copy would split the reviews, the saves, and the ranking, and a reader would have no way to tell which one to trust.
  ...
  - ⏳ **The 40 recipes already in the catalog need `npm run backfill:source-hash -- --apply` before this covers them.** They carry a source URL but no hash, so until it runs, pasting one of their URLs extracts a duplicate. The script is a dry run unless `--apply` is passed.
  - ⚠️ *Deduplication is against the public catalog only.* A user with the same recipe already on their own shelf gets a second draft. Shelf-level deduplication is a separate question and is not in this scope.
