...
- **Private Extraction Shelf:** A per-user view of recipes the user extracted but has not published. Visible only to that user. Fully usable — readable, printable, addable to the shopping list — without ever being published.
  ...
  - ✅ *Drafts are editable before publishing.* `/shelf/:id/edit` reuses `ManualRecipeForm` rather than growing a second editor. A parser gets ingredients subtly wrong often enough that an uneditable draft is not much of a draft. Published recipes are not editable there — the shelf holds only a reference and the catalog copy is canonical.
  - ✅ *The "Tried & True" sign-off moved to the publish gate.* It used to block saving a draft, which is the wrong moment: saving to the shelf is collecting, publishing is vouching. It now sits in `PublishPanel` beside the photo, and both are required. The server still validates `triedAndTrue` independently.
...
- ✅ **Dual Attribution:** Every published recipe credits two parties, both visibly. `publishRecipe.js` resolves the publisher's display name when the recipe is published and stores it as `publishedByName`; `Recipe.jsx` renders it as a byline under the title. The Phase 2 Source Attribution Component still credits the original creator with a live link at the foot of the page, where it keeps earning them referral clicks. The SSR JSON-LD carries both halves: the publisher as a `Person` author, the source page as `isBasedOn`.
  - *The name is resolved at publish time, not at read time.* `firestore.rules` lets a user read only their own `users/{uid}` document, so a reader cannot turn a uid into a name. Denormalising the name onto the recipe is what makes the credit visible at all.
  - *A name containing `@` is never stored.* `recipes` is world-readable and `ssrRecipe` serialises the whole document into public page source, so an email reaching that field would be published. The uid is fine there and stays; it is opaque.
  - *A failed lookup returns null instead of failing the publish.* A missing byline is cosmetic. A publish that died on one would cost the user their photo upload and their Tried & True sign-off.
  - *The client cannot set its own byline.* The server writes `publishedByName` after spreading the submitted recipe, so a crafted payload is overwritten rather than trusted.
  - ⚠️ *Every recipe in the catalog today predates this.* All 40 carry no `publishedByUid` and no name, so they keep the `Organization` author in their schema. Nothing backfills them, and nothing can: there is no uid on those documents to resolve.
  - `functions/ssrRecipe.test.js` drives the real `ssrRecipe` handler over a stubbed Firestore and checks both credits reach the rendered page, including that neither can close the JSON-LD `<script>` tag.
- ✅ **Ratings & Reviews (display only):** Stars and text on the recipe page, restricted to authenticated, email-verified users. `src/services/reviewService.js` stores them at `reviews/{slug}/entries/{uid}`, and `src/components/Reviews/` renders the summary, the list, and the one form a signed-in reviewer gets.
  - *A review changes nothing about visibility.* No ranking, no homepage shelf, no trusted-poster badge, and no `aggregateRating` in the JSON-LD — rich-result stars are a visibility payoff too, and the whole point is that a sockpuppet account buys nothing. **Do not add ranking to this phase.**
  - *Keyed by slug, not by document id*, because a server-rendered recipe page hydrates from the document's fields and never learns its Firestore id. Keyed by uid underneath, which makes one review per account per recipe structural rather than a rule to enforce.
  - *The author name is pinned by `firestore.rules` to the name the identity provider issued* (`request.auth.token.name`), so a review cannot be signed with someone else's. An account with no name shows as "A Recifree cook".
  - *The average is computed from the reviews on the page*, not denormalised onto the recipe. A stored counter would need a transaction to stay honest, and it is the first step towards ranking.
  - `npm run prove:reviews` checks the rules against the emulator: 13 checks covering the happy path, both auth gates, rating bounds, the text limit, unexpected fields, name spoofing, and cross-account writes and deletes.
  - ⚠️ *Local dev needs a verified dev user.* `AuthContext` treats `dev@recifree.local` as verified so the form appears, but the rules read `request.auth.token.email_verified`, which the emulator sets to false. Flip it with the Admin SDK against the auth emulator or the write is denied.
  - ⏳ *Review text is a moderation surface* and joins titles, descriptions, and instructions under Text moderation below.
- ✅ **Paste Deduplication:** A URL that already maps to a published recipe never produces a second one. `publishRecipe.js` stores `sourceUrlHash` on every recipe it publishes; `functions/publishedLookup.js` matches a paste against it, and `extractRecipe.js` answers `{ duplicate: true, slug }` instead of extracting. `AddRecipe` routes the user to that recipe and adds it to their saved list, and the recipe page says why they landed there.
  - *The check runs before the page is fetched*, so a duplicate paste costs one indexed query — no fetch, no parse, no Gemini call.
  - *The key is the same normalized URL hash the extraction cache uses*, imported rather than reimplemented. A second copy would split the reviews, the saves, and the ranking, and a reader would have no way to tell which one to trust.
  - *`normalizeUrl` collapses everything that is not the page's identity:* `http` and `https`, `www.` and the bare domain, host casing, the default port, a trailing slash, a trailing `/amp` from a mobile share, the fragment, query order, and every tracking parameter — whole families by prefix (`utm_`, `mtm_`, `pk_`, `hsa_`) plus the click ids that follow no pattern. It deliberately leaves path casing and non-`www` subdomains alone, because either can be a genuinely different page.
  - *The paste is not always the page's address.* After the fetch, `extractRecipe` also matches on where a redirect landed and on the publisher's own `<link rel="canonical">`, which is how a Pinterest link, a shortener, and a `?p=1234` permalink all find the recipe they point at. The canonical is only trusted same-host, so a misconfigured tag cannot route a reader to someone else's recipe. A cache hit runs the same check against the cached source URL, because it returns before the page is fetched.
  - *A recipe is attributed to the address the page answers to*, not to the shortener that was pasted, so the original creator gets the real link and a later paste has something to match.
  - *The user's own shelf is deduplicated too.* Re-pasting a URL already on the shelf opens that draft instead of the editor; saving from the editor overwrites the draft by id, which would silently take any edits with it. Published references are skipped — they hold a pointer, not a recipe.
  - *A failed lookup extracts as normal.* A duplicate is worse than an extra parse; a dead paste is worse than both.
  - *The save is guarded against the toggle.* `toggleSaved` is a toggle, so pasting a URL already in the user's saved list would have removed it.
  - *Every write path sets the key.* `publishRecipe` computes it, and so does `scripts/migrate-to-firestore.js`, which is how a recipe added through `.agent/workflows/recipe.md` reaches Firestore. `scripts/backfill-source-hash.js` remains the audit and the repair.
  - ⛔ **Changing `normalizeUrl` invalidates every stored key.** The function starts producing new-style hashes while the catalog still holds old-style ones, and pastes silently stop matching. Deploy, then immediately run `npm run backfill:source-hash -- --rehash --apply`. As of the normalizer hardening, 19 of 41 catalog recipes change key.
  - ⚠️ *Deduplication is against the public catalog and the user's own shelf.* It cannot see another user's shelf, by design, and it cannot tell that two different URLs are the same dish.
...
- ✅ **Scope the Extraction Cache to Prevent Leaks:** `functions/cache/extractionCache.js` keys purely on a SHA-256 of the normalized URL with no user scoping, and returns the cached document to any caller. It used to carry `_extractionMeta.extractedBy` — the extracting user's email — which would have been served to the next user pasting the same URL. `extractRecipe.js` no longer records it, and `stripExtractorIdentity()` removes it on both read and write so the documents written before the guard existed are scrubbed too. Covered by four tests in `functions/cache/extractionCache.test.js`, all of which fail if the guard is removed.
  ...
  - ✅ The one production document that still stored an email at rest has been scrubbed. `npm run scrub:extractor-identity` deletes the field, and with no `--apply` it audits instead — expect it to find nothing.
- ✅ **Anonymous Rate Limiting:** `functions/security/rateLimiter.js` keys its 10-per-hour limit on whatever `identifyCaller` returns, and `anonymousId()` in `functions/security/callerTier.js` supplies a SHA-256 of the client IP for signed-out callers. `extractRecipe.js` runs the check before any parsing, for every tier.
